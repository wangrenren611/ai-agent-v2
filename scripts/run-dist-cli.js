#!/usr/bin/env node
/**
 * 独立 CLI - 使用编译后的 dist 代码 (CommonJS)
 */
...
async function main() {
    ...
    try {
        ...
        log('gray', `Loading tool module from: ${toolIndex}`);
        const { registerDefaultToolsAsync, ToolRegistry } = require(toolIndex);
        log('gray', 'Tool module loaded');

        log('gray', 'Loading provider module...');
        const { OpenAIProvider } = require(providerIndex);
        log('gray', 'Provider module loaded');

        log('gray', 'Loading agent module...');
        const Agent = require(agentIndex).default;
        log('gray', 'Agent module loaded');

        log('gray', 'Loading prompt module...');
        const { operatorPrompt } = require(promptIndex);
        log('gray', 'Prompt module loaded');
