...
Note: This tool currently supports TypeScript/JavaScript files (.ts, .tsx, .js, .jsx).`;
  ...
  private async goToDefinition(
    ...
    for (const decl of declarations) {
      ...
      const defLineAndChar = declSourceFile.getLineAndCharacterOfPosition(defStart);

      definitions.push({
        fileName: declSourceFile.fileName,
        textSpan: { start: defStart, length: decl.getWidth() },
        name: symbol.getName(),
      });
    }
    ...
    const visitNode = (node: ts.Node): void => {
      ...
      if (
        ...
        if (ts.isFunctionDeclaration(node) && node.name) {
          ...
          name =
            node.declarationList.declarations[0]?.name.getText() || 'anonymous';
