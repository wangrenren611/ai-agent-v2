...
import { ScopedLogger } from './log.js';
...
export class BackupManager {
    ...
    constructor(config: BackupManagerConfig = {}) {
        ...
        this.logger = new ScopedLogger('BackupManager');
    }
    ...
    async initialize(): Promise<void> {
        if (!this.enabled) {
            this.logger.debug('Backup manager is disabled');
            return;
        }

        try {
            await fs.mkdir(this.backupDir, { recursive: true });
            this.logger.debug(`Backup directory ready: ${this.backupDir}`);
        } catch (error) {
    ...
    async backup(filePath: string): Promise<string | null> {
        ...
        try {
            ...
            if (!stats || !stats.isFile()) {
                this.logger.debug(`File not found, skipping backup: ${filePath}`);
                return null;
            }
            ...
            this.logger.debug(`Backed up: ${filePath} -> ${backupId}`);
            return backupId;
    ...
    async restore(filePath: string, backupId: string): Promise<boolean> {
        if (!this.enabled) {
            this.logger.warn('Backup manager is disabled, cannot restore');
            return false;
        }
        ...
        try {
            ...
            if (!backups) {
                this.logger.error(`No backups found for: ${filePath}`);
                return false;
            }
            ...
            this.logger.error(`Restore failed for ${filePath}: ${error}`);
            return false;
        }
    }
    ...
    async deleteBackup(filePath: string, backupId: string): Promise<void> {
        ...
        if (backupInfo) {
            ...
            this.logger.debug(`Deleted backup: ${backupId}`);
        }
    }
    ...
    async clean(filePath: string): Promise<void> {
        ...
        this.logger.debug(`Cleared all backups for: ${filePath}`);
    }
