...
const TokenUsage: React.FC<TokenUsageProps> = ({ usedTokens, totalTokens }) => {
  const percentage = Math.round((usedTokens / totalTokens) * 100);
  const color = percentage > 80 ? COLORS.error : percentage > 60 ? COLORS.warning : COLORS.muted;

  return (
    <Box flexDirection="row" marginTop={1} marginLeft={2}>
      <Text dimColor>
        Tokens: <Text color={color}>{usedTokens.toLocaleString()}</Text> / {totalTokens.toLocaleString()} ({percentage}%)
      </Text>
    </Box>
  );
};
  ...
  return (
    <Box flexDirection="column" width="100%" paddingX={1}>
      ...
      <Box flexDirection="column" marginTop={1}>
        ...
        {usedTokens && (
          <TokenUsage usedTokens={usedTokens.usedTokens} totalTokens={usedTokens.totalTokens} />
        )}
      </Box>
    </Box>
  );
