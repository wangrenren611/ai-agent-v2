...
interface SearchableScrollableSelectProps extends ScrollableSelectProps {
  ...
  searchQuery?: string;
  /** 自定义过滤函数 */
  filterFunction?: (item: SelectItem, query: string) => boolean;
}

/**
 * 默认过滤函数
 */
export const defaultFilter = (item: SelectItem, query: string): boolean => {
  const lowerQuery = query.toLowerCase();
  return item.label.toLowerCase().includes(lowerQuery);
};

/**
 * SearchableScrollableSelect 组件
 *
 * @example
 * ```tsx
 ...
 *   items={commands}
 *   searchQuery={input}
 *   filterFunction={(item, query) => item.label.includes(query)}
 *   onSelect={handleSelect}
 * />
 * ```
 */
export const SearchableScrollableSelect: React.FC<SearchableScrollableSelectProps> = ({
  items,
  searchQuery = '',
  filterFunction,
  ...restProps
  ...
  return <ScrollableSelect items={filteredItems} {...restProps} />;
};
