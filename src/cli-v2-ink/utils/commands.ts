...
export function matchCommands(keyword: string): Command[] {
  console.log('[matchCommands] Called with keyword:', JSON.stringify(keyword));
  
  if (!keyword || keyword === '/') {
    console.log('[matchCommands] Returning all commands:', COMMANDS.length);
    return COMMANDS;
  }
  ...
  console.log('[matchCommands] Filtered commands:', filtered.length);
  return filtered;
}
...
export function findCommand(name: string): Command | undefined {
  console.log('[findCommand] Called with name:', name);
  const found = COMMANDS.find(cmd => cmd.name.toLowerCase() === name.toLowerCase());
  console.log('[findCommand] Found:', found ? found.name : 'not found');
  return found;
}
